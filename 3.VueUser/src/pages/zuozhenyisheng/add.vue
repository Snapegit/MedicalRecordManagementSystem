...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="医龄" prop="yiling">
            <el-input v-model="ruleForm.yiling" 
                placeholder="医龄" clearable :disabled=" false  ||ro.yiling"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="擅长" prop="shanzhang">
            <el-input v-model="ruleForm.shanzhang" 
                placeholder="擅长" clearable :disabled=" false  ||ro.shanzhang"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="挂号费用" prop="guahaofeiyong">
            <el-input v-model.number="ruleForm.guahaofeiyong" 
                placeholder="挂号费用" clearable :disabled=" false  ||ro.guahaofeiyong"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="坐诊时间" prop="zuozhenshijian">
            <el-input v-model="ruleForm.zuozhenshijian" 
                placeholder="坐诊时间" clearable :disabled=" false  ||ro.zuozhenshijian"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zuozhenyisheng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zuozhenyisheng/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zuozhenyisheng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
