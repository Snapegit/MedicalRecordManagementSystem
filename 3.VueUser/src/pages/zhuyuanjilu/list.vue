...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>床位编号：</div>
        <el-input v-model="formSearch.chuangweibianhao" placeholder="床位编号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>科室：</div>
	  ...
	  <el-button v-if="btnAuth('zhuyuanjilu','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"0px","background":"#c5c5c5","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/zhuyuanjiluAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"padding":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"320px"}' class="img-box">
				<img @click.stop="imgPreView(item.chuangweitupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.chuangweitupian && item.chuangweitupian.substr(0,4)=='http'" :src="item.chuangweitupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.chuangweitupian?item.chuangweitupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.chuangweitupian?item.chuangweitupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">床位名称:{{item.chuangweimingcheng}}</div>
				<div class="title">科室:{{item.keshi}}</div>
		        ...
		        <div :style='{"padding":"2px 0px","borderColor":"#f0f0f0","margin":"0 10px 0 0","borderStyle":"dashed","borderWidth":"0 0 0px 0","display":"inline-block"}' class="publisher">
		          ...
		          <span class="text">{{item.yishengzhanghao}}</span>
		        </div>
		      </div>
		      <div class="desc" v-html="item.zhuyishixiang"></div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.chuangweibianhao != '') searchWhere.chuangweibianhao = '%' + this.formSearch.chuangweibianhao + '%';
        if (this.formSearch.keshi != '') searchWhere.keshi = '%' + this.formSearch.keshi + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`zhuyuanjilu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zhuyuanjiluDetail', query: params});
      },
