...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="床位编号" prop="chuangweibianhao">
              <el-input v-model="ruleForm.chuangweibianhao" placeholder="床位编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="床位状态" prop="chuangweizhuangtai">
            <el-select v-model="ruleForm.chuangweizhuangtai" placeholder="请选择床位状态" :disabled="true  ||ro.chuangweizhuangtai" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="床位费用" prop="chuangweifeiyong">
            <el-input v-model.number="ruleForm.chuangweifeiyong" 
  ...
  export default {
    data() {
      ...
      return {
        ...
        ruleForm: {
          chuangweibianhao: this.getUUID(),
          chuangweimingcheng: '',
          keshi: '',
          chuangweizhuangtai: '空闲' ,
          chuangweifeiyong: '',
        },
        keshiOptions: [],
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chuangweixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			if(this.ruleForm.chuangweibianhao){
				this.ruleForm.chuangweibianhao = String(this.ruleForm.chuangweibianhao)
			}
			//更新跨表属性
			var crossuserid;
			var crossrefid;
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chuangweixinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chuangweixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
