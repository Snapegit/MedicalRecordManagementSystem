<template>
	<div class="main-content" :style='{"width":"100%","padding":"30px","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">预约编号</label>
						<el-input v-model="searchForm.yuyuebianhao" placeholder="预约编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="取药状态" prop="quyaozhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">取药状态</label>
						<el-select clearable v-model="searchForm.quyaozhuangtai" placeholder="请选择取药状态" >
							<el-option v-for="(item,index) in quyaozhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('chufangjilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#8a99b2","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('chufangjilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu8" :style='{"margin":"0 2px","fontSize":"14px","color":"#8e9db5","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('chufangjilu','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"#8b9ab3","height":"40px"}'></span>
						批量支付
					</el-button>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('chufangjilu','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('chufangjilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('chufangjilu','取药')" @click="quyaojiluCrossAddOrUpdateHandler(scope.row,'cross','否','是','quyaozhuangtai','已取药','已取药,待取药'.split(',')[0])" type="success">
							...
							<el-button class="edit" v-if=" isAuth('chufangjilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('chufangjilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<quyaojilu-cross-add-or-update v-if="quyaojiluCrossAddOrUpdateFlag" :parent="this" ref="quyaojiluCrossaddOrUpdate"></quyaojilu-cross-add-or-update>
...
import quyaojiluCrossAddOrUpdate from "../quyaojilu/add-or-update";
	export default {
		data() {
			return {
				...
				quyaojiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			quyaojiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.quyaojiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','chufangjilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quyaojiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quyaojiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','chufangjilu');
    ...
    init () {
		this.isPayOptions = "已支付,未支付".split(',')
          this.quyaozhuangtaiOptions = "已取药,待取药".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    params['sort'] = 'kaijushijian';
    params['order'] = 'desc';
           if(this.searchForm.quyaozhuangtai!='' && this.searchForm.quyaozhuangtai!=undefined){
            params['quyaozhuangtai'] = this.searchForm.quyaozhuangtai
          }
