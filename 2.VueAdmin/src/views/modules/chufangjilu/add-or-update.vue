	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"50px 0 30px 0","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="取药状态" prop="quyaozhuangtai">
					<el-select :disabled="ro.quyaozhuangtai" v-model="ruleForm.quyaozhuangtai" placeholder="请选择取药状态" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="取药状态" prop="quyaozhuangtai">
					<el-input v-model="ruleForm.quyaozhuangtai"
						placeholder="取药状态" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="病症诊断" prop="bingzhengzhenduan">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.bingzhengzhenduan" label="病症诊断" prop="bingzhengzhenduan">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.bingzhengzhenduan}}</span>
				</el-form-item>
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				this.ruleForm.quyaozhuangtai = '待取药'; 
			}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `chufangjilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chufangjiluCrossAddOrUpdateFlag = false;
											this.parent.search();
