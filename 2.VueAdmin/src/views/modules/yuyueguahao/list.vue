<template>
	<div class="main-content" :style='{"width":"100%","padding":"30px","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="zhaopian" width="200" label="照片">
						<template slot-scope="scope">
							<div v-if="scope.row.zhaopian">
								<img v-if="scope.row.zhaopian.substring(0,4)=='http'" :src="scope.row.zhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.zhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuyueguahao','取消')" @click="guahaoquxiaoCrossAddOrUpdateHandler(scope.row,'cross','否','否','[1]','已取消')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('yuyueguahao','病历登记')" @click="dianzibingliCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"32px"}'></span>
								病历登记
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueguahao','开具处方')" @click="chufangjiluCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"32px"}'></span>
								开具处方
							</el-button>
		...
		<guahaoquxiao-cross-add-or-update v-if="guahaoquxiaoCrossAddOrUpdateFlag" :parent="this" ref="guahaoquxiaoCrossaddOrUpdate"></guahaoquxiao-cross-add-or-update>
		<dianzibingli-cross-add-or-update v-if="dianzibingliCrossAddOrUpdateFlag" :parent="this" ref="dianzibingliCrossaddOrUpdate"></dianzibingli-cross-add-or-update>
		<chufangjilu-cross-add-or-update v-if="chufangjiluCrossAddOrUpdateFlag" :parent="this" ref="chufangjiluCrossaddOrUpdate"></chufangjilu-cross-add-or-update>
...
import guahaoquxiaoCrossAddOrUpdate from "../guahaoquxiao/add-or-update";
import dianzibingliCrossAddOrUpdate from "../dianzibingli/add-or-update";
import chufangjiluCrossAddOrUpdate from "../chufangjilu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				guahaoquxiaoCrossAddOrUpdateFlag: false,
				dianzibingliCrossAddOrUpdateFlag: false,
				chufangjiluCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			guahaoquxiaoCrossAddOrUpdate,
			dianzibingliCrossAddOrUpdate,
			chufangjiluCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			guahaoquxiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.guahaoquxiaoCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.guahaoquxiaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.guahaoquxiaoCrossaddOrUpdate.init(row.id,type);
				});
			},
			dianzibingliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.dianzibingliCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueguahao');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.dianzibingliCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.dianzibingliCrossaddOrUpdate.init(row.id,type);
				});
			},
			chufangjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.chufangjiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueguahao');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chufangjiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chufangjiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
