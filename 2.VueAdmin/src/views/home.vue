...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"rgb(255, 255, 255)","height":"auto"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":"2px solid #cdd4de","padding":"5px 0","margin":"0 10px","borderRadius":"40px","display":"flex"}' v-if="isAuth('dianzibingli','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#8d9cb5","fontWeight":"bold","height":"24px"}'>{{dianzibingliCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#8d9cb5","height":"24px"}'>电子病历总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="dianzibingliChart1" class="echarts1" v-if="isAuth('dianzibingli','首页统计')"></div>
		<div id="dianzibingliChart2" class="echarts2" v-if="isAuth('dianzibingli','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getdianzibingliCount();
		this.dianzibingliChat1();
		this.dianzibingliChat2();
	},
	methods:{
		// 词云
		...
		getdianzibingliCount() {
			this.$http({
				url: `dianzibingli/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.dianzibingliCount = data.data
				}
			})
		},
		dianzibingliChat1() {
        ...
        var dianzibingliChart1 = echarts.init(document.getElementById("dianzibingliChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                dianzibingliChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    dianzibingliChart1.resize();
                };
            }
        });
      })
    ...
    dianzibingliChat2() {
      this.$nextTick(()=>{
        ...
        var dianzibingliChart2 = echarts.init(document.getElementById("dianzibingliChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                dianzibingliChart2.setOption(option);
				
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, dianzibingliChart2)
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    dianzibingliChart2.resize();
                };
            }
        });
      })
